...
public class MohastEntity extends BaseWaterEntity implements GeoEntity, SmartBrainOwner<MohastEntity> {
	...
	@Override
	public List<ExtendedSensor<MohastEntity>> getSensors() {
		return ObjectArrayList.of(new NearbyPlayersSensor<>(), new NearbyLivingEntitySensor<MohastEntity>().setPredicate((target, entity) -> target instanceof Player || !(target instanceof BaseWaterEntity) || target instanceof Villager), new HurtBySensor<>(), new UnreachableTargetSensor<MohastEntity>());
	}

	@Override
	public BrainActivityGroup<MohastEntity> getCoreTasks() {
		return BrainActivityGroup.coreTasks(new LookAtTarget<>(), new MoveToWalkTarget<>());
	}

	@Override
	public BrainActivityGroup<MohastEntity> getIdleTasks() {
		return BrainActivityGroup.idleTasks(new FirstApplicableBehaviour<MohastEntity>(new TargetOrRetaliate<>(), new SetPlayerLookTarget<>().stopIf(target -> !target.isAlive() || (target instanceof Player player && (player.isCreative() || player.isSpectator())))), new OneRandomBehaviour<>(new SetRandomWalkTarget<>().speedModifier(1), new Idle<>().runFor(entity -> entity.getRandom().nextInt(30, 60))));
	}

	@Override
	public BrainActivityGroup<MohastEntity> getFightTasks() {
	...
	public void tick() {
		...
		if (!level().isClientSide && getCurrentAttackType() == AttackType.NONE)
			setCurrentAttackType(AttackType.ATTACK);
	}
