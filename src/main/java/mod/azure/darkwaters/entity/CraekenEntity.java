...
public class CraekenEntity extends BaseWaterEntity implements GeoEntity, SmartBrainOwner<CraekenEntity> {
	...
	public List<ExtendedSensor<CraekenEntity>> getSensors() {
		return ObjectArrayList.of(new NearbyPlayersSensor<>(), new NearbyLivingEntitySensor<CraekenEntity>().setPredicate((target, entity) -> target instanceof Player || !(target instanceof BaseWaterEntity) || target instanceof Villager), new HurtBySensor<>(), new UnreachableTargetSensor<CraekenEntity>());
	}
	...
	public BrainActivityGroup<CraekenEntity> getIdleTasks() {
		return BrainActivityGroup.idleTasks(new FirstApplicableBehaviour<CraekenEntity>(new TargetOrRetaliate<>(), new SetPlayerLookTarget<>().stopIf(target -> !target.isAlive() || (target instanceof Player player && (player.isCreative() || player.isSpectator()))), new SetRandomLookTarget<>()), new OneRandomBehaviour<>(new SetRandomWalkTarget<>().speedModifier(1), new Idle<>().runFor(entity -> entity.getRandom().nextInt(30, 60))));
	}
	...
	public BrainActivityGroup<CraekenEntity> getFightTasks() {
		return BrainActivityGroup.fightTasks(
				...
				new WaterMeleeAttack<>(10).whenStarting(entity -> setAggressive(true)).whenStarting(entity -> setAggressive(false)));
	}
	...
	public void registerControllers(ControllerRegistrar controllers) {
		...
		}).triggerableAnim("attack", RawAnimation.begin().then("attack", LoopType.PLAY_ONCE)).triggerableAnim("grab", RawAnimation.begin().then("grab", LoopType.PLAY_ONCE)).triggerableAnim("death", RawAnimation.begin().thenPlayAndHold("death")));
	}
