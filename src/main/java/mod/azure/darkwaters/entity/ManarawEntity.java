...
public class ManarawEntity extends BaseWaterEntity implements GeoEntity, SmartBrainOwner<ManarawEntity> {
	...
	public ManarawEntity(EntityType<? extends BaseWaterEntity> entityType, Level world) {
		...
		this.xpReward = DarkWatersConfig.manaraw_exp;
	}

	public static AttributeSupplier.Builder createMobAttributes() {
		return BaseWaterEntity.createMobAttributes().add(Attributes.MAX_HEALTH, DarkWatersConfig.manaraw_health)
				.add(Attributes.ATTACK_DAMAGE, DarkWatersConfig.manaraw_attack_damage);
	}
	...
	public List<ExtendedSensor<ManarawEntity>> getSensors() {
		return ObjectArrayList.of(new NearbyPlayersSensor<>(),
				new NearbyLivingEntitySensor<ManarawEntity>().setPredicate((target, entity) -> target instanceof Player
						|| !(target instanceof BaseWaterEntity) || target instanceof Villager),
				new HurtBySensor<>(), new UnreachableTargetSensor<ManarawEntity>());
	}
	...
	public BrainActivityGroup<ManarawEntity> getIdleTasks() {
		return BrainActivityGroup.idleTasks(
				new FirstApplicableBehaviour<ManarawEntity>(new TargetOrRetaliate<>(),
	...
	public void registerControllers(ControllerRegistrar controllers) {
		controllers.add(new AnimationController<>(this, "idle_controller", 0, event -> {
			...
			if (this.entityData.get(STATE) == 1 && !(this.dead || this.getHealth() < 0.01 || this.isDeadOrDying()))
				return event.setAndContinue(RawAnimation.begin().then("engulf_begin", LoopType.PLAY_ONCE)
						.then("engulf_close", LoopType.LOOP));
